# psr15-guard advanced app

Production-shaped guarded PSR-15 app: env-driven `SecurityConfig` engine
...
- `public/index.php`: front controller (engine assembly, guard, emitter)
- `src/Config.php`: env-driven `SecurityConfig` plus the admin gate closure
- `src/Routes.php`: guarded `RequestHandlerInterface` router
...
| `REDIS_PREFIX` | `guard_core_psr15:` | Key namespace for bans and rate limits |
...
4. `GET /` with `X-Forwarded-For: 203.0.113.7` (trusted compose network resolves it) is blocked `403` with body `Blocked by psr15-guard`
