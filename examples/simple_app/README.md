...
`RenzoFranceschini\GuardCorePsr15\GuardMiddleware` (the psr15-guard adapter for
...
- `customErrorResponses`: `403` body is `Blocked by psr15-guard`
- `excludePaths`: `/health`
...
4. XSS query param (`?q=<script>alert(1)</script>`) trips the `xss` threat ban (threshold 1): `403` with body `Blocked by psr15-guard`
5. The same IP is now Redis-banned: `GET /` is blocked `403` with body `Blocked by psr15-guard`
